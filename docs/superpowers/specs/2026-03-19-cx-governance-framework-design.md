...
- The CO maintains the registry index (adds/removes entries when Cx roles publish or retire guidance)
...
- **CEO autonomy monitoring:** CO verifies the CEO is not acting beyond its granted autonomy (as tracked in the executive brief). If a violation is detected: work stops immediately, control returns to user, critical compliance finding logged.
