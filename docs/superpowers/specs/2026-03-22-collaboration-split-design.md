...
**Extracted from existing compiler (~100 lines):**
...
**New (not present as extractable functions in existing compiler):**
...
| `compile_log <level> <message> [args]`  | Structured compiler logging (new — the existing compiler has `log_violation`/`log_pass` but these live inside a generated heredoc in `generate_enforce()`, not as standalone functions) |
| `emit_header <compiler_name> <sources>` | Standard "GENERATED by X from Y" traceability header (new function, inspired by the pattern in `generate_prose`)                                                                        |
...
- **Grounded in existing code** — manifest/verify functions are extracted directly from the working compliance compiler. New functions (frontmatter parsing, logging, header emission) are inspired by patterns in the existing compiler but are new implementations.
