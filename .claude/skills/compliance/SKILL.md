...
Manage the compliance program through the Compliance Officer. All changes to the compliance floor and targets go through this skill.
...
1. Read compliance state: `compliance-floor.md` (count rules), `.claude/compliance/targets.md` (count targets), `.claude/compliance/change-log.md` (recent activity), `.claude/findings/register.md` (compliance-related findings).
...
4. Log `compliance-proposed` event via `ops/metrics-log.sh`. Notify CO of pending proposal.

## Workflow: Review

1. Load the proposal file.
2. Dispatch CO agent (Opus). CO classifies as Type 1/2/3, assesses risk.
3. Cx consultation: CO consults each active Cx role (CISO) for domain impact. Build consensus.
4. Decision gate: Type 1 with consensus -> CO approves, notifies user. Type 2-3 or no consensus -> present to user.
...
2. Present results. Findings automatically copied to CO.
